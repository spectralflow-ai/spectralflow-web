...
> = {
  tensor: {
    title: "Four diamond chips against the spacecraft's own field",
    simple:
      "Spacecraft magnetometry has carried the same burden for sixty years: a long boom to escape the vehicle's own magnetic noise. Four tiny diamond chips centimetres apart measure how steeply the field changes, and steepness is the fingerprint of nearby sources: the vehicle is steep, a planet is flat. The platform's noise is rejected by construction, no boom required.",
    deep:
      "A localised source at range r produces a dipole field, a homogeneous function of degree −3. Euler's identity then gives G·r = −3·B_dip: the field gradient G of a near source is large and structured, while the ambient planetary field is quasi-uniform (G ≈ 0) across a 3 cm baseline. Four vector chips give 12 measurements for 8 unknowns (uniform field, 3; symmetric trace-free gradient, 5), an over-determined linear system constrained by Maxwell (div B = 0). The spacecraft's field collapses into the gradient by construction.\n\nWhy this matters in orbit: ESA's Swarm satellites carry a 4.3 m boom for exactly this problem, and even the 16U NanoMagSat design keeps a deployable mast. Software-only cleaning of boomless cubesat magnetometers exists as post-hoc machine learning; this is not that. This is a hardware gradient measurement plus physical source attribution with calibrated confidence, and the same array yields vector, scalar and full gradient-tensor data products simultaneously, at room temperature, with no consumables and diamond's native radiation hardness. Model-derived at this stage; the architecture is patent-filed (GB2615137.3).",
    refs:
      "Patent GB2615137.3; ESA Swarm (4.3 m boom); NanoMagSat mission design.",
  },
  map: {
