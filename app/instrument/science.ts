...
export const TOPICS: Record<TopicKey, ScienceTopic> = {
  ...
  drift: {
    ...
    simple:
      "An inertial unit measures turn rates and accelerations, then adds tiny errors thousands of times per second. Those errors compound: the longer you fly blind, the faster the position error grows. Each accepted magnetic fix resets the solution, and the filter even learns the heading drift between fixes. That is the sawtooth on the blue curve: drift up, snap down.",
    deep:
      "The dominant term here is uncompensated gyro bias drift: a heading error that grows linearly in time makes the cross-track error grow quadratically, e(t) ≈ V·(δθ₀·t + ½·β·t²). The red curve is that open-loop reference (each world draws β between 0.004 and 0.007 deg/s), ending kilometres off after a ten-minute leg. The blue track runs closed-loop, the way the real chain runs: every ACCEPTED fix recentres the dead-reckoning solution and shifts the filter's belief with it, and consecutive corrections give an estimate of the residual heading-rate error that is trimmed out, a one-parameter observer on the drift.\n\nWithheld fixes commit nothing: no recentring, no belief update. During an attack the blue curve therefore climbs at inertial slope until the first accepted fix snaps it back, which is exactly the honest cost of refusing corrupted measurements. Model-derived.",
    refs:
