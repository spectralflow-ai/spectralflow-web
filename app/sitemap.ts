...
export default function sitemap(): MetadataRoute.Sitemap {
  const core = ["", "/technology", "/applications", "/instrument", "/tools", "/company", "/news", "/contact", "/legal", "/privacy"];
  const verticals = VERTICAL_SLUGS.map((slug) => `/applications/${slug}`);

  const entry = (path: string): MetadataRoute.Sitemap[number] => ({
    url: `${BASE}${path}`,
    changeFrequency: "monthly",
    priority: path === "" ? 1 : path === "/instrument" || path.startsWith("/applications/") ? 0.8 : 0.7,
  });
